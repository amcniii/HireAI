...
export default function Compare() {
  ...
  const handleCompareSelected = async () => {
    ...
    try {
      ...
      setViewMode("compare");
      toast.success("Side-by-side comparison loaded");
    } catch (error) {
      console.error(error);
                ...
                <tbody>
                  {candidates.slice((currentPage - 1) * 10, currentPage * 10).map((candidate, index) => {
                    ...
                    return (
                      ...
                      <td>
                        <span className={globalIndex === 0 ? "status open" : "status closed"} style={{ whiteSpace: "nowrap", display: "inline-block", fontWeight: "normal" }}>
                          {globalIndex === 0 ? "Best Match" : "Compare"}
                        </span>
                      </td>
